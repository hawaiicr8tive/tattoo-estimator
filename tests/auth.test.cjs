...
;(async () => {
  ...
  await test('guest cannot reach research, controls, or spend money', () => {
    ...
    for (const denied of ['page:research', 'page:phrases', 'images:generate', 'images:bulk', 'library:curate', 'users:manage']) {
  ...
  await test('a new member sees every page but not user management', () => {
    ...
    for (const page of perms.PAGE_ORDER) {
      if (page.permission === 'users:manage') continue
      assert.ok(p.has(page.permission), `member cannot reach ${page.href}`)
    }
    // /users is itself a nav page, gated on users:manage — a member must not
    // see the tab or reach the page.
    assert.ok(!p.has('users:manage'), 'member can manage users')
    assert.strictEqual(perms.permissionForPath('/users'), 'users:manage')
    assert.ok(!perms.PAGE_ORDER.filter(x => p.has(x.permission)).some(x => x.href === '/users'))
  })
  ...
  await test('firstAllowedPage skips pages the user lacks', () => {
    assert.strictEqual(perms.firstAllowedPage(new Set(['page:phrases'])), '/phrases')
  ...
  await test('permissionForPath maps every nav page and rejects others', () => {
    ...
    assert.strictEqual(perms.permissionForPath('/phrases'), 'page:phrases')
    assert.strictEqual(perms.permissionForPath('/phrases/'), 'page:phrases')
  ...
  await test('a guest is 403ed from paid actions but not from allowed pages', async () => {
    ...
    const token = encode(auth.userSessionPayload(user))
    await withStoredUser(user, async () => {
      for (const denied of ['images:generate', 'images:bulk', 'users:manage', 'page:phrases']) {
