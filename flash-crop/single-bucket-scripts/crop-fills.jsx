/*
 * crop-fills.jsx  —  bucket: crop-fills/  (paper fills the frame, art centered)
 * Centered square crop, full short-edge. Art untouched.
 * MANUAL FALLBACK: copy this into the crop-fills folder, then
 *   Photoshop ▸ File ▸ Scripts ▸ Browse… ▸ pick it.  Output → squared/ subfolder.
 * (run-all.mjs normally does this for you via crop-core.jsx — this is the by-hand copy.)
 */
#target photoshop
(function () {
  app.displayDialogs = DialogModes.NO
  var ZOOM = 1.0, MAX_EDGE = 1600, JPG_QUALITY = 10
  run(ZOOM, MAX_EDGE, JPG_QUALITY)

  function run(ZOOM, MAX_EDGE, JPG_QUALITY) {
    ...
    for (var i = 0; i < files.length; i++) {
      try { one(files[i], out, ZOOM, MAX_EDGE, JPG_QUALITY); done++ }
      catch (e) { fail++; log.push(files[i].name + ' :: ' + e) }
    }
    app.preferences.rulerUnits = u
    alert('crop-fills done: ' + done + '   failed: ' + fail + (log.length ? '\n\n' + log.join('\n') : '') + '\n\nSaved to: ' + out.fsName)
  }
  function one(file, out, ZOOM, MAX_EDGE, JPG_QUALITY) {
