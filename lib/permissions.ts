 ...
 * server components, route handlers, and client components alike — the Users
 * screen renders the same catalog the server enforces.
 */
...
export const PERMISSION_META: Record<Permission, { label: string; description: string; group: string }> = {
  'page:dashboard':  { group: 'Pages',   label: 'Dashboard',       description: 'Trend cycle dashboard and Flash Designs.' },
  ...
  'page:phrases':    { group: 'Pages',   label: 'Phrases',         description: 'Edit the chaos phrase library.' },
...
export const ROLE_DEFAULTS: Record<Role, readonly Permission[]> = {
  ...
  admin: PERMISSIONS,
  // Everything the owner can do except manage users. Individual permissions
  // can still be switched off per person.
  member: [
    'page:dashboard',
    'page:trends',
    'page:research',
 ...
 * `/users` sits last so it's only ever a fallback for an admin with nothing
 * else switched on.
 */
export const PAGE_ORDER: readonly { href: string; permission: Permission; label: string }[] = [
  ...
  { href: '/phrases',  permission: 'page:phrases',   label: 'Phrases' },
  { href: '/users',    permission: 'users:manage',   label: 'Users' },
]

/** Page permission for a pathname, or null if the path isn't a gated page. */
export function permissionForPath(pathname: string): Permission | null {
  const path = pathname.replace(/\/+$/, '') || '/'
  return PAGE_ORDER.find(p => p.href === path)?.permission ?? null
}
